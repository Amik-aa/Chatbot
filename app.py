...
class chatbot:
    def __init__(self,root):
        ...
        #===========title================
        lbl_title = Label(self.root,bg='green',text='Chatbot',font=('arial',25,'bold'))
 ...
 # ======================================functions===============  #
    ...
    def send(self):
        ...
        input_list = ['hi','hey','hello there','how are you']
        output_list = ['hi','I am good','great to see you','hello']
        for mess in input_list:
            if self.entry.get() == mess:
                put_into_text = random.choice(output_list)
                self.text.insert(END,'\n\n'+"Bot :"+put_into_text)

        data_science="data science is a growing field these days. It has many \n"\
        "applications in daily life. it has the following sub-fields \n1: machine learning\n2: deep learning \n3: NLP"
        if self.entry.get()=="what is data science" or self.entry.get()=="about data science":
            self.text.insert(END,'\n\n'+data_science)
        google = "Google is a tech giant.\nchrome is the leading app of google."
        if self.entry.get()=="google" or self.entry.get()=="about google":
            self.text.insert(END,'\n\n'+google)

        if self.entry.get()=="richest person in the world" or self.entry.get()=="richest person":
            self.text.insert(END,'\n\n'+"Jeff Bezos")
